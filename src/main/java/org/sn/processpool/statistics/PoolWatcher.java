...
public class PoolWatcher extends AbstractHandler
{
        ...
        throws IOException, ServletException
    {
        ...
        response.getWriter().println("<div>PROCESS STATUS FOR POOL :"+_es.getPoolName() +"</div>");
        response.getWriter().println("<table>");
        for(ProcessHolder processHolder : content){
        	response.getWriter().println("<tr>");
        	response.getWriter().println("<td><div >"+processHolder.toString()+"</div></td>");
        	response.getWriter().println("</tr>");
        }
        response.getWriter().println("</table>");
        response.getWriter().println("</html>");
	...
	public static void  watchPool(ExecutorService es, int port) throws Exception{
		_es = es;
		_port = port;
		startServer();
	}
